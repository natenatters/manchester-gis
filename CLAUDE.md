...
**Building files:** Each building gets its own JSON file in `data/projects/example/buildings/`. Hard-code unique polygon geometry for each building - do NOT use generic templates. Study photos to understand:
- Tower position (west end? central? corner?)
- Nave/body shape (rectangular? cruciform? aisles?)
- Wings, transepts, porticos, porches
- Relative proportions

**IMPORTANT:** Only edit individual building files in `buildings/`. Don't touch `building_entities.json` - it's auto-generated by a consolidation hook.

**Map positions:** Buildings need positions for each historical map layer. The Berry 1650 map isn't geo-correct (others are).
...
```json
{
  ...
  "rotation": -5,
  "maps": {
    "berry_1650": { "center": [...], "rotation": 0 },
    "berry_1750": { "center": [...], "rotation": 354, "scale": 0.8 },
    "modern": { "center": [...], "rotation": 0 }
  }
}
```
